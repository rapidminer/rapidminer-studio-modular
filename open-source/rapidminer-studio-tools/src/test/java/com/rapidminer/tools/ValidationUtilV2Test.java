...
/**
 * Tests for {@link ValidationUtilV2}
 *
 * @author Jan Czogalla
 * @since 9.3
 */
public class ValidationUtilV2Test {
	...
	/**
	 * Run {@link ValidationUtilV2#dependencySortEmptyListForLoops(Function, Collection)} and check results;
	 * <p>
	...
	private void testListSort(Map<String, List<String>> dependency, List<String> expectedResult) {
		...
		try {
			sortedValues = ValidationUtilV2.dependencySortNoLoops(
	...
	/** Run {@link ValidationUtilV2#dependencySortEmptyListForLoops(Function, Collection)} and compare result */
	private void testListSortSuppressed(Map<String, List<String>> dependency, List<String> expectedResult) {
		List<String> sortedValues = null;
		sortedValues = ValidationUtilV2.dependencySortEmptyListForLoops(
				k -> new HashSet<>(dependency.getOrDefault(k, Collections.emptyList())), stringValues);
		assertEquals(expectedResult, sortedValues);
	}
}