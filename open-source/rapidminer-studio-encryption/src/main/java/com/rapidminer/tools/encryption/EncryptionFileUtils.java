...
enum EncryptionFileUtils {
	...
	List<KeysetHandleContainer> loadAllKeysetHandlesFromDisk() {
		if (!canAccessFileSystem()) {
			LogService.getRoot().config(() -> "No file access allowed, skipping loading of keysets");
		...
		for (EncryptionType encryptionType : EncryptionType.values()) {
			...
			for (Path encryptionFile : files) {
				String context = FileUtils.removeSuffix(encryptionFile.getFileName().toString());
				try {
	...
	void storeOnDisk(KeysetHandle keysetHandle, String context, EncryptionType encryptionType) throws IOException {
		if (!canAccessFileSystem()) {
			LogService.getRoot().config(() -> "No file access allowed, skipping storing on disk");
			return;
		}
		// try to delete first
	...
	void deleteFromDisk(String context, EncryptionType encryptionType) throws IOException {
		if (!canAccessFileSystem()) {
			LogService.getRoot().config(() -> "No file access allowed, skipping deletion from disk");
			return;
		}
	...
	/**
	 * Checks whether file system access is allowed.
	 */
	private static boolean canAccessFileSystem() {
		return Boolean.parseBoolean(Settings.getSetting(SettingsConstants.EXECUTION_CAN_ACCESS_FILESYSTEM));
	}
}
