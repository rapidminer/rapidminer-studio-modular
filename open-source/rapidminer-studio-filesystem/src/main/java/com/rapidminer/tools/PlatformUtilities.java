...
public final class PlatformUtilities {
	...
	private static void initializeReleaseVersion() {
		synchronized (INIT_VERSION_LOCK) {
			...
			if (currentVersion == null) {
				...
				if (currentVersion == null) {
					logInfo("Could not read current version from properties file. Looking for 'rapidminer.platform.version' settings...");
					currentVersion = Settings.getSetting(SettingsConstants.PLATFORM_VERSION);
					if (currentVersion == null) {
						throw new IllegalStateException("Could not initialize current version.");
					}
				} else {
					logInfo("Found version " + currentVersion + " in gradle.properties, using it.");
				}
			}
		}
	}
	 ...
	 * {@link #ensureRapidMinerHomeSet(boolean)} with {@code true}.
	 */
	public static void ensureRapidMinerHomeSet() {
		ensureRapidMinerHomeSet(true);
	}
	 ...
	 */
	public static synchronized void ensureRapidMinerHomeSet(final boolean log) {
	...
	private static boolean searchInClassPath() {
		...
		for (String pathComponent : pathComponents) {
			String path = pathComponent.trim();
			/*
	...
	private static File retrieveRMHomeFromLibraryDir(File buildDir) {
		...
		if (projectDir != null && projectDir.getName().endsWith("rapidminer-studio-filesystem")) {
			projectDir = projectDir.getParentFile();
			// now we are in open-source folder, need to go up one mor level
			projectDir = projectDir.getParentFile();
			if (projectDir != null) {
				projectDir = projectDir.toPath().resolve("rapidminer-studio-core").toAbsolutePath().toFile();
			}
		}
	...
	private static void logInfo(String msg) {
		LogService.getRoot().log(Level.INFO, msg);
	}

	private static void logWarn(String msg) {
		LogService.getRoot().log(Level.WARNING, msg);
	}

	private static void logSevere(String msg) {
		LogService.getRoot().log(Level.SEVERE, msg);
	}
