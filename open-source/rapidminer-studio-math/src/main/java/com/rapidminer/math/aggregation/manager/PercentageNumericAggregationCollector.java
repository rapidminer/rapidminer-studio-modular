...
/**
 * An aggregation collector that collects numeric values and then divides by the sum and multiplies by 100.
 *
 * @author Gisa Meier
 * @since 9.1
 */
class PercentageNumericAggregationCollector implements AggregationCollector {

	private final NumericBuffer buffer;
	private double totalSum;

	PercentageNumericAggregationCollector(int numberOfRows) {
	...
	public Column getResult(Context context) {
		for (int i = 0; i < buffer.size(); i++) {
			buffer.set(i, buffer.get(i) / totalSum * 100);
		}
