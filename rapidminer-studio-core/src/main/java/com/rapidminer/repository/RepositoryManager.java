...
public class RepositoryManager extends AbstractObservable<Repository> {
	...
	public static void init() {
		synchronized (INSTANCE_LOCK) {
			...
			CleanupHooks.addCleanupHook(RepositoryManager::cleanup);
	 ...
	 * {@link RapidMiner#init()} after {@link com.rapidminer.tools.plugin.Plugin#initAll()} and {@link RepositoryManager#init()}
	 * .
	 */
	@SuppressWarnings("unchecked")
	...
	public void createRepositoryIfNoneIsDefined() {
		...
		if (noLocalRepository) {
			try {
				Path repositoryPath = FilesystemRepositoriesTools.getDefaultRepositoryFolder(localRepositoryAlias).toPath();
		...
		if (isFolder) {
			...
			if (destination.isSupportingBinaryEntries()) {
				...
				try (InputStream in = blob.openInputStream(); OutputStream out = targetEntry.openOutputStream()) {
					IOTools.copyStreamSynchronously(in, out, false);
					if (listener != null) {
						listener.setCompleted(maxProgress);
					}
				} catch (IOException e) {
