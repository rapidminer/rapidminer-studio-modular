...
import com.rapidminer.repository.FilesystemRepositoriesTools;
...
public class FilesystemRepositoryFactory implements CustomRepositoryFactory {
	...
	public Repository fromXML(Element element) throws RepositoryException, XMLException {
		String xmlpath = XMLTools.getTagContents(element, "path", true);
		String path = FilesystemRepositoriesTools.getUnshortenedDocsPath(xmlpath);
	...
	public static Element toXml(FilesystemRepositoryAdapter repositoryAdapter, Document doc) {
		...
		file.appendChild(doc.createTextNode(FilesystemRepositoriesTools.getShortenedDocsPath(repositoryAdapter.getRoot())));
	 ...
	 *                          encrypting files in that repo. See {@link EncryptionProvider}.
	...
	public static Repository createRepository(String alias, Path targetPath, boolean editable, boolean isTransient, String encryptionContext) throws RepositoryException {
		...
		try {
			FilesystemRepositoryAdapter repo = new FilesystemRepositoryAdapter(alias, targetPath.toString(), editable, isTransient, encryptionContext);
			RepositoryManager.getInstance(null).addRepository(repo);
			return repo;
		} catch (FileNotFoundException e) {
			throw new RepositoryException(e);
		}
