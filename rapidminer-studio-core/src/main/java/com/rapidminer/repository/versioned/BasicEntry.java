...
public abstract class BasicEntry<T> extends RepositoryFile<T> implements Entry {
	...
	/**
	 * Checks whether the data summary of this file is cached in memory so that it can be retrieved instantly.
	 *
	 * @return {@code true} if the data summary is cached in memory
	 * @since 9.8.0
	 */
	protected boolean hasCachedDataSummary() {
		return getRepositoryAdapter().hasDataSummaryCached(this);
	}
}
