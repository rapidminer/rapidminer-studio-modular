...
public class BasicIOObjectEntry extends AbstractIOObjectEntry<IOObject> {
    ...
    protected void setIOObjectData(IOObject data) throws RepositoryFileException, RepositoryImmutableException,
            RepositoryException {
        if (data != null) {
            // prevent specialized IOOs from being written to default ioo format
            Class<? extends IOObjectEntry> entryClass = IOObjectEntryTypeRegistry.getEntryClassForIOObjectClass(data.getClass());
            if (entryClass != BasicIOObjectEntry.class) {
                throw new RepositoryException("Data must not be " + data.getClass().getSimpleName() + "!");
            }
        }
        setData(data);
    }
