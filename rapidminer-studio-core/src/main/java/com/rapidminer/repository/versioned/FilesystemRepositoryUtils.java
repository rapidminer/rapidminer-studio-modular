...
public enum FilesystemRepositoryUtils {
	...
	public static void addTextualDiffSuffix(String suffix) {
		TEXT_DIFF_SUFFIXES.add(normalizeSuffix(ValidationUtilV2.requireNonNull(StringUtils.stripToNull(suffix), "suffix")));
	}
	...
	public static boolean isPathTextuallyDiffable(Path path) {
		return TEXT_DIFF_SUFFIXES.contains(normalizeSuffix(FileUtils.getSuffixFromFilename(
				ValidationUtilV2.requireNonNull(path, "path").getFileName().toString())));
	}

	/**
	 * Checks the given {@link RepositoryFileException} cause recursively to find a {@link RepositoryException} if
	 * possible. If a cause was found, returns that, otherwise wraps the given exception in a {@link
	 * RepositoryException}.
	 *
	 * @param e the exception to handle
	 * @return the corresponding repository exception
	 * @since 9.8
	 */
	static RepositoryException handleRepoFileException(RepositoryFileException e) {
		// try to unpack cause
