...
public class InjectParametersModel {
	...
	public InjectParametersModel(List<ConnectionParameterModel> parameters, List<ValueProvider> valueProviders) {
		this.originalParameters = parameters != null ? parameters.stream().filter(Objects::nonNull)
				.map(ConnectionParameterModel::copyDataOnly).collect(Collectors.toList()) : Collections.emptyList();
