...
public class ConnectionModel {
	...
	private final Map<String, StringProperty> transientProperties = Collections.synchronizedMap(new HashMap<>());
	 ...
	 */
	public StringProperty getTransientProperty(String propertyName) {
		return transientProperties.get(propertyName);
	}

	/**
	 * Gets or creates the transient property
	 *
	 * @param propertyName the name of the parameter
	 * @return the parameter, or {@code null}
	 */
	public StringProperty getOrCreateTransientProperty(String propertyName, String value) {
		StringProperty property = getTransientProperty(propertyName);
		if (property == null) {
			property = new SimpleStringProperty(value);
			transientProperties.put(propertyName, property);
		}
		return property;
	}
