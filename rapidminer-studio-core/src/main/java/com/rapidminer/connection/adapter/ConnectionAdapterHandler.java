...
public abstract class ConnectionAdapterHandler<T extends ConnectionAdapter>
	...
	public T getAdapter(ConnectionInformation connection, Operator operator) throws ConnectionAdapterException, ConfigurationException {
		...
		boolean cacheAdapter = operator != null && operator.getProcess() != null;
		if (cacheAdapter) {
			try {
		...
		Map<String, String> adapterMap = valueMap.entrySet().stream().filter(e ->  e.getValue() != null)
				.collect(Collectors.toMap(e -> e.getKey().substring(e.getKey().indexOf('.') + 1),
						e -> encrypt(e.getValue(), keyMap.get(e.getKey()))));
		if (cacheAdapter) {
	 ...
	 * 		{@link ParameterUtility#isValueSet(ConfigurationParameter)} are marked as
	...
	protected ValidationResult validate(Function<String, String> parameterValues, Function<String, ConfigurationParameter> configParameters) {
		...
		parametersByGroup().forEach((group, names) -> names.forEach(name -> {
			...
			if (parameter == null || !ParameterUtility.isValueSet(parameter)) {
	...
	private static final ParameterTypePassword PASSWORD_CONVERTER = new ParameterTypePassword("", "");

	private static String encrypt(String value, ConfigurationParameter parameter) {
		if (parameter == null || !parameter.isEncrypted()) {
			return value;
		}
		return PASSWORD_CONVERTER.toXMLString(value, EncryptionProvider.DEFAULT_CONTEXT);
	}
	//endregion
