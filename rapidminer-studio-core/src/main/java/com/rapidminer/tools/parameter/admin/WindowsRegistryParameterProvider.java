...
public class WindowsRegistryParameterProvider implements ParameterProvider {
	...
	/**
	 * System property which prevents loading of the jnidispatch native library from the PATH
	 *
	 * @see com.sun.jna.Native
	 */
	private static final String JNA_NOSYS = "jna.nosys";

	static {
		// Set jna.nosys to true, if not set externally, to prevent jnidispatch version conflicts
		if (IS_WINDOWS && System.getProperty(JNA_NOSYS) == null) {
			System.setProperty(JNA_NOSYS, Boolean.TRUE.toString());
		}
	}
