...
/**
 * A {@link com.rapidminer.tools.belt.expression.Function} for replacing parts of a string by an other. Which parts
 * replaced parts is defined by a regular expression
 *
 * @author David Arnu
 * @since 9.11
 */
public class ReplaceAll extends AbstractArbitraryStringInputStringOutputFunction {
	...
	protected String compute(String text, String regex, String replacement) {
		...
		if (text == null || regex == null || replacement == null) {
			...
			if (regex.length() == 0) {
				throw new FunctionInputExceptionWrapper("expression_parser.function_missing_arguments", "regex", getFunctionName());
			}
	...
	protected void checkNumberOfInputs(int inputLength) {
		if (inputLength != 3) {
			throw new FunctionInputExceptionWrapper("expression_parser.function_wrong_input", getFunctionName(), 3, inputLength);
		}
	}
