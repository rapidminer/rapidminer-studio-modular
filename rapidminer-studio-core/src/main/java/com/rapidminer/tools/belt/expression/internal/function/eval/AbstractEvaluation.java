...
abstract class AbstractEvaluation extends AbstractFunction {
	...
	/**
	 * Checks if the computation can take place, called at the beginning of
	 * {@link #compute(Callable, ExpressionContext, ExpressionEvaluator...)} (ExpressionEvaluator...)}.
	 */
	...
	@Override
	public ExpressionEvaluator compute(Callable<Void> stopChecker, ExpressionContext context, final ExpressionEvaluator... inputEvaluators) {
		checkSetup();
		...
		if (inputEvaluators.length == 1) {
			...
			if (!inputEvaluators[0].isConstant()) {
				throw new FunctionInputExceptionWrapper("expression_parser.eval.non_constant_single_argument",
						getFunctionName());
			}
			// if eval has one constant argument compute it once using the parser
			return compute(context, inputEvaluators[0]);
			...
			if (inputEvaluators[0].isConstant()) {
				...
				return computeAndCheckType(context, inputEvaluators[0], getExpectedReturnType(inputEvaluators[1]));
			} else {
				...
				switch (expectedType) {
					case INSTANT:
						return makeDateEvaluator(context, expectedType, inputEvaluators[0]);
					case DOUBLE:
					case INTEGER:
						return makeDoubleEvaluator(context, expectedType, inputEvaluators[0]);
					case BOOLEAN:
						return makeBooleanEvaluator(context, expectedType, inputEvaluators[0]);
					case STRING:
					default:
						return makeStringEvaluator(context, expectedType, inputEvaluators[0]);
				}
			}
	 ...
	 */
	private ExpressionEvaluator compute(ExpressionContext context, ExpressionEvaluator subexpressionEvaluator) {
		...
		return compute(context, expressionString);
	}
	 ...
	 */
	protected abstract ExpressionEvaluator compute(ExpressionContext context, String expressionString);
	 ...
	 * @return the result of parsing the expression string
	 */
	private ExpressionEvaluator computeAndCheckType(ExpressionContext context, ExpressionEvaluator subexpressionEvaluator,
													ExpressionType expectedType) {
		ExpressionEvaluator outEvaluator = compute(context, subexpressionEvaluator);
		if (outEvaluator.getType() == expectedType) {
			...
			throw new FunctionInputExceptionWrapper("expression_parser.eval.type_not_matching", getFunctionName(),
	...
	private String getStringValue(ExpressionEvaluator outEvaluator) throws Exception {
		switch (outEvaluator.getType()) {
			...
			case INSTANT:
				return dateToString(outEvaluator.getInstantFunction().call());
			default:
				// cannot happen
				return null;
		}
	 ...
	 */
	private String dateToString(Instant input) {
		if (input == null) {
			return null;
		} else {
	...
	private ExpressionType getExpectedReturnType(ExpressionEvaluator expressionEvaluator) {
		if (!expressionEvaluator.isConstant()) {
			...
			throw new FunctionInputExceptionWrapper("expression_parser.eval.type_not_constant", getFunctionName(),
					validTypeArguments);
		}
		...
		if (expectedType == null) {
			String validTypeArguments = TypeConstants.INSTANCE.getValidConstantsString();
			throw new FunctionInputExceptionWrapper("expression_parser.eval.invalid_type", typeString, getFunctionName(),
					validTypeArguments);
		}
		return expectedType;
	}
	 ...
	 * {@link #computeAndCheckType(ExpressionContext, ExpressionEvaluator, ExpressionType)}.
	 */
	private ExpressionEvaluator makeDateEvaluator(ExpressionContext context, final ExpressionType expectedType,
												  final ExpressionEvaluator inputEvaluator) {
		Callable<Instant> dateCallable = () -> {
			ExpressionEvaluator subExpressionEvaluator = computeAndCheckType(context, inputEvaluator, expectedType);
			return subExpressionEvaluator.getInstantFunction().call();
		};
		return ExpressionEvaluatorFactory.ofInstant(dateCallable, false);
	}
	 ...
	 * {@link #computeAndCheckType(ExpressionContext, ExpressionEvaluator, ExpressionType)}.
	 */
	private ExpressionEvaluator makeBooleanEvaluator(ExpressionContext context, final ExpressionType expectedType,
													 final ExpressionEvaluator inputEvaluator) {
	 ...
	 * {@link #computeAndCheckType(ExpressionContext, ExpressionEvaluator, ExpressionType)}.
	 */
	private ExpressionEvaluator makeDoubleEvaluator(ExpressionContext context, final ExpressionType expectedType,
													final ExpressionEvaluator inputEvaluator) {
	 ...
	 * {@link #computeAndCheckType(ExpressionContext, ExpressionEvaluator, ExpressionType)}.
	 */
	private ExpressionEvaluator makeStringEvaluator(ExpressionContext context, final ExpressionType expectedType,
													final ExpressionEvaluator inputEvaluator) {
