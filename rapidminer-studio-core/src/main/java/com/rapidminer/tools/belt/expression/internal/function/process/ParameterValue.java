 ...
 */
package com.rapidminer.tools.belt.expression.internal.function.process;
...
public class ParameterValue extends AbstractArbitraryStringInputStringOutputFunction {
	...
	protected void checkNumberOfInputs(int length) {
		...
		if (length != expectedInput) {
			throw new FunctionInputExceptionWrapper("expression_parser.function_wrong_input", getFunctionName(), expectedInput,
					length);
		}
	}
	...
	protected String compute(String... values) {
		try {
			...
			if (operator == null) {
				throw new FunctionInputExceptionWrapper("expression_parser.parameter_value_wrong_operator", getFunctionName());
			}
			return operator.getParameter(values[1]);
		} catch (UndefinedParameterError e) {
			throw new FunctionInputExceptionWrapper("expression_parser.parameter_value_wrong_parameter", getFunctionName());
		}
	}
