...
/**
 * Abstract class for a {@link com.rapidminer.tools.belt.expression.Function} that has arbitrary many arguments.
 *
 * @author David Arnu
 * @since 9.11
 */
public abstract class AbstractArbitraryDoubleInputFunction extends AbstractFunction {
	...
	public AbstractArbitraryDoubleInputFunction(String i18n, int numberOfArgumentsToCheck, ExpressionType returnType) {
	...
	public ExpressionEvaluator compute(Callable<Void> stopChecker, ExpressionContext context,
		...
		if (inputEvaluators.length < 1) {
			throw new FunctionInputExceptionWrapper("expression_parser.function_wrong_minimum_input", getFunctionName(), 1,
					inputEvaluators.length);
		}
		ExpressionType type = getResultType(inputEvaluators);
		return ExpressionEvaluatorFactory.ofDouble(makeDoubleCallable(inputEvaluators), isResultConstant(inputEvaluators), type);
	}
