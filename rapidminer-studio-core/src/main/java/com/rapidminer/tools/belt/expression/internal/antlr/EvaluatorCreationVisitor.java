...
import com.rapidminer.tools.expression.internal.antlr.FunctionExpressionParserBaseVisitor;
...
class EvaluatorCreationVisitor extends FunctionExpressionParserBaseVisitor<ExpressionEvaluator> {
	...
	public ExpressionEvaluator visitOperationExp(OperationExpContext ctx) {
		if (ctx.op == null) {
			...
			if (function == null) {
				throw new UnknownFunctionExceptionWrapper(ctx, "expression_parser.unknown_operator", operatorName);
			}
			return function.compute(getStopChecker(), lookUp, evals);
		}
	}
	...
	public ExpressionEvaluator visitFunction(FunctionContext ctx) {
		...
		if (function == null) {
			throw new UnknownFunctionExceptionWrapper(ctx, "expression_parser.unknown_function", functionName);
		}

		return function.compute(getStopChecker(), lookUp, innerEvaluators);
	}
	...
	public ExpressionEvaluator visitAttribute(AttributeContext ctx) {
		...
		if (attributeEvaluator == null) {
			throw new UnknownDynamicVariableExceptionWrapper(ctx, "expression_parser.unknown_attribute", attributeName);
		}
	...
	public ExpressionEvaluator visitVariable(VariableContext ctx) {
		...
		if (variableEvaluator == null) {
			throw new UnknownVariableExceptionWrapper(ctx, "expression_parser.unknown_variable", name);
		}
	...
	public ExpressionEvaluator visitScopeConstant(ScopeConstantContext ctx) {
		...
		if (scopeConstantEvaluator == null) {
			throw new UnknownScopeConstantExceptionWrapper(ctx, "expression_parser.unknown_scope", scopeConstantName);
		}
	...
	public ExpressionEvaluator visitIndirectScopeConstant(IndirectScopeConstantContext ctx) {
		...
		ExpressionEvaluator attributeEvaluator = lookUp.getDynamicVariable(attributeName);
		if (attributeEvaluator == null) {
			throw new UnknownDynamicVariableExceptionWrapper(ctx, "expression_parser.unknown_attribute_in_scope", attributeName,
					scopeConstantName);
		}
		return attributeEvaluator;
	}
	...
	public ExpressionEvaluator visitReal(RealContext ctx) {
		...
		return ExpressionEvaluatorFactory.ofDouble(doubleValue, ExpressionType.DOUBLE);
	...
	public ExpressionEvaluator visitInteger(IntegerContext ctx) {
		double doubleValue = Double.parseDouble(ctx.getText());
		return ExpressionEvaluatorFactory.ofDouble(doubleValue, ExpressionType.INTEGER);
	}
