...
/**
 * A {@link com.rapidminer.tools.belt.expression.Function} for setting a value of a given date-time.
 *
 * @author Kevin Majchrzak
 * @since 9.11
 */
public class DateTimeSet extends AbstractDateManipulationFunction {
	...
	public DateTimeSet() {
		super("date.date_time_set", 4, ExpressionType.INSTANT);
	}
		...
		try {
			switch (unit) {
				case DATE_TIME_UNIT_YEAR:
					return dateTime.withYear(intValue).toInstant();
				case DATE_TIME_UNIT_MONTH:
					// shifting the range to [0,11] for backwards compatibility
					if (intValue < MIN_MONTH || intValue > MAX_MONTH) {
						throw new FunctionInputExceptionWrapper(I18N_OUT_OF_RANGE,
								"months", MIN_MONTH, MAX_MONTH, intValue);
					}
					return dateTime.withMonth(intValue + 1).toInstant();
				case DATE_TIME_UNIT_WEEK:
					return dateTime.with(ChronoField.ALIGNED_WEEK_OF_YEAR, intValue).toInstant();
				case DATE_TIME_UNIT_DAY:
					return dateTime.withDayOfMonth(intValue).toInstant();
				case DATE_TIME_UNIT_HOUR:
					return dateTime.withHour(intValue).toInstant();
				case DATE_TIME_UNIT_MINUTE:
					return dateTime.withMinute(intValue).toInstant();
				case DATE_TIME_UNIT_SECOND:
					return dateTime.withSecond(intValue).toInstant();
				case DATE_TIME_UNIT_MILLISECOND:
					if (intValue < MIN_MILLI || intValue > MAX_MILLI) {
						throw new FunctionInputExceptionWrapper(I18N_OUT_OF_RANGE,
								"milliseconds", MIN_MILLI, MAX_MILLI, intValue);
					}
					return dateTime.withNano(intValue * MILLI_TO_NANO
							+ dateTime.getNano() % MILLI_TO_NANO).toInstant();
				case DATE_TIME_UNIT_NANOSECOND:
					if (intValue < MIN_NANO || intValue > MAX_NANO) {
						throw new FunctionInputExceptionWrapper(I18N_OUT_OF_RANGE,
								"nanoseconds", MIN_NANO, MAX_NANO, intValue);
					}
					return dateTime.withNano(dateTime.getNano() / MILLI_TO_NANO * MILLI_TO_NANO
							+ intValue).toInstant();
				default:
