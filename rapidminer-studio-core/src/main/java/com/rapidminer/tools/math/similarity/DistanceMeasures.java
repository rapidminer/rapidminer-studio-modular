...
public class DistanceMeasures {
	...
	public static DistanceMeasure createMeasure(ParameterHandler parameterHandler)
		...
		if (measureClass != null) {
			...
			try {
				...
				throw new OperatorException("Could not instantiate distance measure " + measureClass);
			}
		}
		return null;
	}
	...
	/**
	 * Returns two maps, let's call them "map1" and "map2", that map the original nominal attributes' index values to
	 * index values of a common mapping. Can be used for equality checks because {@code map1(index1) == map2(index2) <=>
	 * attribute1.getMapping().mapIndex(index1).equals(attribute2.getMapping().mapIndex(index2))}.
	 *
	 * @since 9.8
	 */
	public static Pair<Map<Double, Integer>, Map<Double, Integer>> createCommonMapping(Attribute attribute1,
																					   Attribute attribute2) {
		Map<Double, Integer> indexMap1 = new HashMap<>();
		Map<Double, Integer> indexMap2 = new HashMap<>();

		NominalMapping mappingAttr1 = attribute1.getMapping();
		NominalMapping mappingAttr2 = attribute2.getMapping();

		Set<String> values = new HashSet<>(mappingAttr1.getValues());
		...
		for (String value : values) {
			...
			if (valueIndexAttr1 != -1 && valueIndexAttr2 != -1) {
				indexMap1.put((double) valueIndexAttr1, index);
				indexMap2.put((double) valueIndexAttr2, index);

				index++;
			}
		}

		return new Pair<>(indexMap1, indexMap2);
	}
