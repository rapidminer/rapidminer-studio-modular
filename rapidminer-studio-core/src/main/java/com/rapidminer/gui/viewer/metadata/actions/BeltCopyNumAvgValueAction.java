 ...
 * @since 9.7.0
 */
public class BeltCopyNumAvgValueAction extends ResourceAction {

	private static final long serialVersionUID = 1L;

	/**
	 * Creates a new {@link BeltCopyNumAvgValueAction} instance.
	 */
	public BeltCopyNumAvgValueAction() {
	...
	public void loggedActionPerformed(ActionEvent e) {
		...
		if (model.getType() == AbstractBeltColumnStatisticsModel.Type.TIME) {
			Tools.copyStringToClipboard(Objects.toString(((BeltTimeColumnStatisticsModel) model).getAverage(), "?"));
		} else {
			Tools.copyStringToClipboard(String.valueOf(((BeltNumericalColumnStatisticsModel) model).getAverage()));
		}
	}
