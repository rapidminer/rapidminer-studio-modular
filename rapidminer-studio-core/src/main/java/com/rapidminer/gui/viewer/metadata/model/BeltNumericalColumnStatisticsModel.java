...
/**
 * Model for {@link BeltColumnStatisticsPanel}s which are backed by a numerical {@link com.rapidminer.belt.column.Column}.
 *
 * @author Marco Boeck, Gisa Meier
 * @since 9.7.0
 */
public class BeltNumericalColumnStatisticsModel extends AbstractBeltColumnStatisticsModel {
	...
	/**
	 * Creates a new {@link BeltNumericalColumnStatisticsModel}.
	 */
	public BeltNumericalColumnStatisticsModel(Table table, String attribute) {
	...
	public void updateStatistics(Map<String, Map<Statistic, Result>> allStatistics) {
		...
		average = statistics.get(Statistic.MEAN).getNumeric();
		deviation = statistics.get(Statistic.SD).getNumeric();
		minimum = statistics.get(Statistic.MIN).getNumeric();
		maximum = statistics.get(Statistic.MAX).getNumeric();
	 ...
	 */
	private HistogramDataset createHistogramDataset(Table table) {
		HistogramDataset dataset = new HistogramDataset();

		double[] array = new double[table.height()];
		int count = 0;

		NumericReader reader = Readers.numericReader(table.column(getColumnName()));
