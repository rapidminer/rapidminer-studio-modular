 ...
 * @since 9.7.0
 */
public class BeltCopyNumMaximumValueAction extends ResourceAction {

	private static final long serialVersionUID = 1L;

	/**
	 * Creates a new {@link BeltCopyNumMaximumValueAction} instance.
	 */
	public BeltCopyNumMaximumValueAction() {
	...
	public void loggedActionPerformed(ActionEvent e) {
		...
		if (model.getType() == AbstractBeltColumnStatisticsModel.Type.TIME) {
			Tools.copyStringToClipboard(Objects.toString(((BeltTimeColumnStatisticsModel) model).getMaximum(), "?"));
		} else {
			Tools.copyStringToClipboard(String.valueOf(((BeltNumericalColumnStatisticsModel) model).getMaximum()));
		}
	}
