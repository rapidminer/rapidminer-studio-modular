 ...
 * @since 9.7.0
 */
public class BeltCopyNumDeviationValueAction extends ResourceAction {

	private static final long serialVersionUID = 1L;

	/**
	 * Creates a new {@link BeltCopyNumDeviationValueAction} instance.
	 */
	public BeltCopyNumDeviationValueAction() {
	...
	public void loggedActionPerformed(ActionEvent e) {
		...
		if (model.getType() == AbstractBeltColumnStatisticsModel.Type.TIME) {
			Tools.copyStringToClipboard(Objects.toString(((BeltTimeColumnStatisticsModel) model).getDeviation(), "?"));
		} else {
			Tools.copyStringToClipboard(String.valueOf(((BeltNumericalColumnStatisticsModel) model).getDeviation()));
		}
	}
