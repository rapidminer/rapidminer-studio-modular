...
public class FilterTableModel extends AbstractTableModel implements TablePanelModel {
	...
	public FilterTableModel(InputPort inputPort) throws IllegalArgumentException {
		...
		final ExampleSetMetaData metaData = inputPort.getMetaDataAsOrNull(ExampleSetMetaData.class);
		if (metaData != null) {
			this.md = metaData;
	...
	public List<String> convertEncodedStringValueToList(String encodedValue) {
		List<String> result;
		try {
			result = StringTools.unescape(encodedValue, CustomFilters.ESCAPE_CHAR, new char[] { CustomFilters.SEPERATOR_CHAR },
	...
	public String encodeListOfStringsToValue(List<String> listOfStrings) {
		...
		for (int i = 0; i < listOfStrings.size(); i++) {
			...
			if (listOfStrings.get(i) != null) {
				builder.append(StringTools.escape(listOfStrings.get(i), CustomFilters.ESCAPE_CHAR,
