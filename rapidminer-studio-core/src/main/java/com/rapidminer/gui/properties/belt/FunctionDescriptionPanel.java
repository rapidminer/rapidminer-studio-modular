...
public class FunctionDescriptionPanel extends JPanel {
	...
	private static final long serialVersionUID = -8887583386441504891L;
	...
	private transient FunctionDescription functionEntry;
	...
	private void updateFunctionEntry(final FunctionDescription functionEntry) {
		...
		Icon icon = AttributeGuiTools.getIconForExpressionType(functionEntry.getReturnType(), true);
		lblReturnTypeIcon.setIcon(icon);
		lblReturnTypeIcon.setToolTipText(mapExpressionTypeToDisplayName(functionEntry.getReturnType()));
	...
	private void updateHeight() {
		...
		if (isVisible() && textareaInfoText.getText() != null && !textareaInfoText.getText().isEmpty()
			...
			int numberOfLines = (int) Math.ceil(getContentHeight(textareaInfoText.getText(), getWidth()) / 16.0);
	...
	/**
	 * Maps the given expression type to the appropriate display name string.
	 */
	static String mapExpressionTypeToDisplayName(ExpressionType returnType) {
		if (returnType == null) {
			return "Unknown type";
		}
		switch (returnType) {
