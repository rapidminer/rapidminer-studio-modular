...
import com.rapidminer.gui.tools.syntax.BeltExpressionTokenMaker;
...
/**
 * The {@link ExpressionPropertyDialog} enables to enter an expression out of functions, variables, constants and
 * macro values and validates the expression's syntax.
 *
...
public class ExpressionPropertyDialog extends PropertyDialog {
	...
	private class PrivateInputObserver implements Observer<FunctionInputPanel> {
		...
		/**
		 * Helper method used to function intputs of category scope in {@link #update(Observable,
		 * FunctionInputPanel)}.
		 */
		private void updateScope(FunctionInputPanel arg) {
			...
			// value or an evaluated expression (only if "eval" function is available in the dialog context)
			if (!predefined) {
				if (parser.getExpressionContext().getFunction("eval") != null) {
	...
	private class PrivateModelObserver implements Observer<FunctionDescription> {
		...
		/**
		 * Adds the given value (function name) to the expression text. If the function has no arguments, the caret is
		 * placed after the function's right parenthesis.
		 */
		private void addToExpression(String value, FunctionDescription function) {
			ExpressionPropertyDialog.this.addToExpression(value);
	...
	// PARSER
	private final transient ExpressionParser parser;
	private final transient Process controllingProcess;
	...
	/** model of the inputs */
	private transient FunctionInputsModel inputsModel = new FunctionInputsModel();

	/** observer to update the GUI, if the model changed or an input value was clicked */
	private transient PrivateInputObserver inputObserver = new PrivateInputObserver();

	/** listener which checks if the value of the filter string changed */
	private transient FilterListener filterInputsListener = value -> inputsModel.setFilterNameString(value);

	/** action which is executed when the filter search field is cleared */
	private final transient ResourceAction clearInputsFilterAction = new ResourceAction(true, "clear_filter") {
	...
	private transient FunctionDescriptionModel functionModel = new FunctionDescriptionModel();

	/** observer to update the GUI, if the model changed or a function was clicked */
	private transient PrivateModelObserver functionObserver = new PrivateModelObserver();

	/** listener which checks if the value of the filter string changed */
	private transient FilterListener filterFunctionsListener = value -> functionModel.setFilterNameString(value);

	/** action which is executed when the filter search field is cleared */
	private final transient ResourceAction clearFunctionsFilterAction = new ResourceAction(true, "clear_filter") {
	 ...
	 */
	public ExpressionPropertyDialog(final ParameterTypeTableExpression type, String initialValue) {
		this(type, null, initialValue);
	}

	/**
	 * Creates an {@link ExpressionPropertyDialog} with the given initial value, controlling process, expression
	 * parser, input model and function model
	 */
	public ExpressionPropertyDialog(ParameterTypeTableExpression type, Process process, List<FunctionInput> inputs,
		...
		BeltExpressionTokenMaker.removeFunctionInputs();
		BeltExpressionTokenMaker.addFunctionInputs(inputs);
		BeltExpressionTokenMaker.addFunctions(functions);
	...
	/**
	 * Creates an {@link ExpressionPropertyDialog} with the given initial value and a controlling process.
	 *
	 * @param type
	 * 		the expression type
	 * @param process
	 ...
	 * 		the initial value
	 */
	public ExpressionPropertyDialog(ParameterTypeTableExpression type, Process process, String initialValue) {
		super(type, "expression");
		...
		InputPort inPort = ((ParameterTypeTableExpression) getParameterType()).getInputPort();
		if (inPort != null) {
			TableMetaData tableMetaData = inPort.getMetaDataAsOrNull(TableMetaData.class);
			if (tableMetaData == null) {
				TableModelMetaData modelMetaData = inPort.getMetaDataAsOrNull(TableModelMetaData.class);
				if (modelMetaData != null) {
					tableMetaData = modelMetaData.getTrainingMetaData();
				}
			}
			if (tableMetaData != null) {
				builder = builder.withDynamics(new TableMetaDataResolver(tableMetaData));
			}
		}
		...
		BeltExpressionTokenMaker.addFunctionInputs(parser.getExpressionContext().getFunctionInputs());
		BeltExpressionTokenMaker.addFunctions(parser.getExpressionContext().getFunctionDescriptions());
	...
	private Process getControllingProcessOrNull(ParameterTypeTableExpression type, Process process) {
		if (process != null) {
			return process;
	...
	public void initGui(String initialValue) {
		...
		currentExpression.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				if (e.getModifiersEx() == CTRL_DOWN_MASK && e.getExtendedKeyCode() == KeyEvent.VK_ENTER) {
					okButton.doClick();
				}
			}
		...
		atmf.putMapping("text/expression", "com.rapidminer.gui.tools.syntax.BeltExpressionTokenMaker");
		...
		// scroll pane containing the different {@link BeltFunctionDescriptionPanel}s
		JScrollPane functionButtonScrollPane = new ExtendedJScrollPane(outerFunctionPanel);
		...
		functionButtonScrollPane.setVerticalScrollBarPolicy(VERTICAL_SCROLLBAR_ALWAYS);
		...
		if (inputsModel.getFilteredModel().size() > 0) {
			...
			filterDropdownButton.addActionListener(createFilterDropDownActionListener(filterMenu, filterDropdownButton));
			...
			inputsScrollPane.setVerticalScrollBarPolicy(VERTICAL_SCROLLBAR_ALWAYS);
		...
		layoutDefault(mainPanel, HUGE, buttons.toArray(new AbstractButton[0]));
	...
	/**
	 * Helper method used in {@link #initGui(String)}.
	 */
	private ActionListener createFilterDropDownActionListener(ScrollableJPopupMenu filterMenu, JButton filterDropdownButton) {
		return e -> {
	...
	private void updateFunctions() {
		...
		Map<String, JXTaskPane> functionCategoryTaskPanes = new HashMap<>();
		GridBagConstraints gbc = new GridBagConstraints();
		...
		for (String functionGroup : filteredModel.keySet()) {
			...
			for (FunctionDescription function : list) {
				...
				if (!perfectMatch && searchStringGiven) {
					...
					if (filterName.equalsIgnoreCase(functionName)
							|| filterName.equalsIgnoreCase(function.getDisplayName())) {
						perfectMatch = true;
					}
				}
			}
			...
			functionCategoryTaskPane.setCollapsed(filteredModel.keySet().size() != 1);
	...
	private void updateInputs() {
		...
		Map<String, JXTaskPane> inputCategoryTaskPanes = new HashMap<>();
		GridBagConstraints gbc = new GridBagConstraints();
		gbc.fill = GridBagConstraints.BOTH;
		gbc.weightx = 1;
		gbc.gridx = 0;
		...
		for (String type : keySet) {
			...
			for (FunctionInput entry : list) {
				...
				if (!perfectMatch && searchStringGiven
						&& filterName.equalsIgnoreCase(entry.getName())) {
					perfectMatch = true;
			...
			// if there is only one category in the filtered model, open the task pane
			inputCategoryTaskPane.setCollapsed(filteredModel.keySet().size() != 1);
		...
		if (totalEntryCount <= MAX_NMBR_INPUTS_SHOWN) {
			...
			if (!anyPerfectMatch) {
				...
				if (filteredModel.get(TableResolver.KEY_ATTRIBUTES) != null
						&& filteredModel.get(TableResolver.KEY_SPECIAL_ATTRIBUTES) != null
						&& filteredModel.get(TableResolver.KEY_ATTRIBUTES).size() + filteredModel
						.get(TableResolver.KEY_SPECIAL_ATTRIBUTES).size() <= MAX_NMBR_INPUTS_SHOWN) {

					inputCategoryTaskPanes.get(TableResolver.KEY_ATTRIBUTES).setCollapsed(false);
					inputCategoryTaskPanes.get(TableResolver.KEY_SPECIAL_ATTRIBUTES).setCollapsed(false);

				} else if (filteredModel.get(TableResolver.KEY_ATTRIBUTES) != null
						&& filteredModel.get(TableResolver.KEY_ATTRIBUTES).size() <= MAX_NMBR_INPUTS_SHOWN) {
	...
	private SyntaxScheme getExpressionColorScheme(SyntaxScheme textAreaSyntaxScheme) {
		...
		textAreaSyntaxScheme.setStyle(SEPARATOR, new Style(DARK_PURPLE));
		// show double quotes / strings in dark cyan
		textAreaSyntaxScheme.setStyle(LITERAL_STRING_DOUBLE_QUOTE, new Style(DARK_CYAN));
		// show attributes in RapidMiner orange
		textAreaSyntaxScheme.setStyle(VARIABLE, new Style(SwingTools.RAPIDMINER_ORANGE));
		// show unknown attributes that are placed in brackets in [] in black
		textAreaSyntaxScheme.setStyle(COMMENT_KEYWORD, new Style(Color.black));
		// show operators that are not defined in the functions in black (like other unknown words)
		textAreaSyntaxScheme.setStyle(OPERATOR, new Style(Color.black));
		return textAreaSyntaxScheme;
	}
	...
	private void showError(boolean error, String title, String message) {
		...
		explanation = (explanation.charAt(0) + "").toUpperCase(Locale.ENGLISH) + explanation.substring(1);
		...
		if (splittedMessage.length > 1) {
			...
			truncateMessage(splittedMessage);
		}
	 ...
	 * a new string array but modifies the given one instead. In this case the error message will contain as much of the
	 * error as possible plus a symmetrical window around it. Parts of the original message that have been truncated are
	 * marked by the TRUNCATED_SYMBOL constant string.
	 *
	 * @param splittedMessage
	 * 		the original error message
	 */
	private static void truncateMessage(String[] splittedMessage) {
		if (splittedMessage.length > 2) {
			truncateMarcedMessage(splittedMessage);
		} else {
			// that means the error has not been marked. therefore we simply cut the end if necessary
			if (ExpressionPropertyDialog.MAX_ERROR_MESSAGE_LENGTH < splittedMessage[1].length()) {
				splittedMessage[1] = splittedMessage[1].substring(0, ExpressionPropertyDialog.MAX_ERROR_MESSAGE_LENGTH) + TRUNCATED_SYMBOL;
			}
		}
	}

	/**
	 * Helper method used in {@link #truncateMessage(String[])} in case the message has been marked. This method avoids
	 * cutting away the error.
	 */
	private static void truncateMarcedMessage(String[] splittedMessage) {
		if (ExpressionPropertyDialog.MAX_ERROR_MESSAGE_LENGTH < splittedMessage[1].length()) {
			...
			if (errorLength > ExpressionPropertyDialog.MAX_ERROR_MESSAGE_LENGTH) {
				// the end of the error has to be cut because there is no space for the whole error
				splittedMessage[1] = splittedMessage[1].substring(errorStart, errorStart + ExpressionPropertyDialog.MAX_ERROR_MESSAGE_LENGTH);
				splittedMessage[2] = splittedMessage[2].substring(errorStart, errorStart + ExpressionPropertyDialog.MAX_ERROR_MESSAGE_LENGTH);
				...
				int rest = ExpressionPropertyDialog.MAX_ERROR_MESSAGE_LENGTH - errorLength;
