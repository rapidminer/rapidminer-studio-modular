...
public class SettingsPropertyPanel extends PropertyPanel {
		...
		for (SettingsItem subGroup : itemSubGroups) {
			for (SettingsItem item : subGroup.getChildren(Type.PARAMETER)) {
				...
				if (!definedKeys.contains(item.getKey())) {
					parameterHandler.setParameter(item.getKey(), parameterType instanceof ParameterTypePassword ?
							parameterType.transformNewValue(getParameterValue(item), EncryptionProvider.DEFAULT_CONTEXT) :
	...
	protected String getValue(ParameterType type) {
		...
		if (value == null) {
			...
			// passwords can be returned directly, they are handled in plaintext in the settings dialog memory
			// this ensures that on each save, the plaintext is encrypted again with a freshly encrypted value on disk
			// this makes it more secure than handling the encrypted value in memory here,
			// which would NOT cause a re-encryption when saving without changing the password
			return type instanceof ParameterTypePassword ? value : type.transformNewValue(value, EncryptionProvider.DEFAULT_CONTEXT);
		}
	}
	...
	protected JPanel createParameterPanel(ParameterType type, PropertyValueCellEditor editor,
		...
		if (editor.rendersLabel()) {
			...
			final JLabel label = new ParameterLabel(type, editorComponent, settingsItem.getTitle(), true);
			...
			contentsPanel.add(label, gbc);
			gbc.gridx++;
			JPanel filler = new JPanel();
			filler.setMinimumSize(new Dimension(20, 0));
			filler.setPreferredSize(filler.getMinimumSize());
			contentsPanel.add(filler, gbc);
			gbc.weightx = 1;
			gbc.gridx++;
			contentsPanel.add(editorComponent, gbc);
