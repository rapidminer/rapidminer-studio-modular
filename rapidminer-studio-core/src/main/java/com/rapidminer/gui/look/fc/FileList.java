...
import static sun.swing.FilePane.VIEWTYPE_DETAILS;
import static sun.swing.FilePane.VIEWTYPE_LIST;
...
public class FileList extends JPanel implements PropertyChangeListener {
	...
	/**
	 * Either the {@link #tablePanel} or the {@link #filePane} is not {@code null} and used for the details view
	 */
	...
	protected JPopupMenu filePanePanePopup;
	...
	protected void changeCardForView() {
		...
		if (filechooserUI.getView().equals(FileChooserUI.FILECHOOSER_VIEW_DETAILS)) {
			...
			if (tablePanel == null && completeItemsList.isEmpty()) {
				//if we have no table panel, we skipped this before
				exploreToList(fc.getCurrentDirectory());
	...
	public FileList(FileChooserUI tfcui, JFileChooser fc, FilePane filePane) {
		this.fc = fc;
	...
	private void init() {
		if (filePane != null) {
			filePane.setViewType(VIEWTYPE_LIST); //this has to be done once due to FilePane's font handling
			filePane.setViewType(VIEWTYPE_DETAILS);
		}
		...
		if (filePane == null) {
			...
			cardPanel.add(filePane, TABLE_SCROLL_PANE);
		}
	...
	/**
	 * @return whether the file pane is displayed.
	 */
	private boolean isShowingFilePane() {
		return filePane != null && FileChooserUI.FILECHOOSER_VIEW_DETAILS.equals(filechooserUI.getView());
	}
	...
	private void exploreToList(File folder) {
	...
	public void requestFocus() {
		if (FileChooserUI.FILECHOOSER_VIEW_DETAILS.equals(filechooserUI.viewType) && tablePanel != null) {
	...
	protected JPopupMenu getPanePopup() {
		if (isShowingFilePane()) {
			if (filePanePanePopup == null) {
				filePanePanePopup = createPanePopup(false);
			}
			return filePanePanePopup;
		} else {
			if (panePopup == null) {
				panePopup = createPanePopup(true);
			}
			return panePopup;
		}
	}

	private JPopupMenu createPanePopup(boolean addSelectionAndSorting) {
		MenuListener menuListener = new MenuListener(this);
		JPopupMenu panePopup = new JPopupMenu();
