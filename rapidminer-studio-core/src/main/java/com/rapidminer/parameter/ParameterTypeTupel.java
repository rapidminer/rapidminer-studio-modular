...
import com.rapidminer.tools.StringTools;
...
public class ParameterTypeTupel extends CombinedParameterType {
	...
	public Element getXML(String key, String value, boolean hideDefault, String encryptionContext, Document doc) {
		...
		for (String part : tupel) {
			...
			valueString.append(StringTools.escape(val, ESCAPE_CHAR, XML_SPECIAL_CHARACTERS));
		}
	...
	public static String[] transformString2Tupel(String parameterValue) {
		...
		List<String> split = StringTools.unescape(parameterValue, ESCAPE_CHAR, INTERNAL_SPECIAL_CHARACTERS,
	...
	public static String transformTupel2String(String firstValue, String secondValue) {
		return StringTools.escape(firstValue, ESCAPE_CHAR, INTERNAL_SPECIAL_CHARACTERS) + INTERNAL_SEPERATOR_CHAR
				+ StringTools.escape(secondValue, ESCAPE_CHAR, INTERNAL_SPECIAL_CHARACTERS);
	}
	...
	public static String transformTupel2String(String[] tupel) {
		...
		for (int i = 0; i < tupel.length; i++) {
			...
			if (tupel[i] != null) {
				builder.append(StringTools.escape(tupel[i], ESCAPE_CHAR, INTERNAL_SPECIAL_CHARACTERS));
			}
		}
	...
	public String transformNewValue(String value) {
		List<String> split = StringTools.unescape(value, ESCAPE_CHAR, XML_SPECIAL_CHARACTERS, XML_SEPERATOR_CHAR);
		...
		for (String part : split) {
			...
			internalEncoded.append(StringTools.escape(part, ESCAPE_CHAR, INTERNAL_SPECIAL_CHARACTERS));
		}
		return internalEncoded.toString();
	}

	public static String escapeForInternalRepresentation(String string) {
		return StringTools.escape(string, ESCAPE_CHAR, INTERNAL_SPECIAL_CHARACTERS);
	}
