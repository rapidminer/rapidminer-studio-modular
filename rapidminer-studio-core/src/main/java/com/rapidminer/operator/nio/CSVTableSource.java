...
/**
 * This operator can be used to load data from .csv files. The user can specify the delimiter and various other
 * parameters. This operator is a copy of {@link CSVExampleSource} and has exactly the same results apart from reading
 * into an {@link IOTable}. For that reason, old compatibility levels are kept.
 *
 * @author Gisa Meier
 * @since 9.9.0
 */
public class CSVTableSource extends AbstractDataResultTableReader {
	...
	static {
		AbstractReader.registerReaderDescription(new ReaderDescription("csv", CSVTableSource.class,
				PARAMETER_CSV_FILE));
	}

	public CSVTableSource(final OperatorDescription description) {
		super(description);
	}
	...
	static void configure(DataSource dataSource, Operator operator) throws DataSetException {
		...
		operator.setParameter(PARAMETER_CSV_FILE, configParameters.get(CSVResultSetConfiguration.CSV_FILE_LOCATION));
		operator.setParameter(PARAMETER_SKIP_COMMENTS,
				configParameters.get(CSVResultSetConfiguration.CSV_SKIP_COMMENTS));
		operator.setParameter(PARAMETER_COMMENT_CHARS,
				configParameters.get(CSVResultSetConfiguration.CSV_COMMENT_CHARACTERS));
		operator.setParameter(PARAMETER_COLUMN_SEPARATORS,
				configParameters.get(CSVResultSetConfiguration.CSV_COLUMN_SEPARATORS));
		operator.setParameter(StrictDecimalFormat.PARAMETER_DECIMAL_CHARACTER,
				configParameters.get(CSVResultSetConfiguration.CSV_DECIMAL_CHARACTER));
		operator.setParameter(Encoding.PARAMETER_ENCODING,
				configParameters.get(CSVResultSetConfiguration.CSV_ENCODING));
		operator.setParameter(PARAMETER_ESCAPE_CHARACTER,
				configParameters.get(CSVResultSetConfiguration.CSV_ESCAPE_CHARACTER));
		operator.setParameter(PARAMETER_USE_QUOTES, configParameters.get(CSVResultSetConfiguration.CSV_USE_QUOTES));
		operator.setParameter(PARAMETER_QUOTES_CHARACTER,
				configParameters.get(CSVResultSetConfiguration.CSV_QUOTE_CHARACTER));
		operator.setParameter(PARAMETER_TRIM_LINES, configParameters.get(CSVResultSetConfiguration.CSV_TRIM_LINES));

		// the backend uses technical row values starting with 0 but the operator parameters show human readable
		// versions thus +1
		...
		operator.setParameter(PARAMETER_FIRST_ROW_AS_NAMES, String.valueOf(
				Boolean.valueOf(configParameters.get(CSVResultSetConfiguration.CSV_HAS_HEADER_ROW)) ||
						headerRowEqualsStartingRow));
		operator.setParameter(PARAMETER_STARTING_ROW, String.valueOf(rowOffset));

		// set meta data
		ImportWizardUtils.setMetaData(dataSource, operator);

		// update compatibility level to latest version
		operator.setCompatibilityLevel(OperatorVersion.getLatestVersion(operator.getOperatorDescription()));
	}
