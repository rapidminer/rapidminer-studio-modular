			...
			public void transformMD() {
				...
				try {
					if (getParameterAsString(PARAMETER_ORDER_MODE).equals(ALPHABETICALLY_MODE)) {
						outputPort.deliverMD(getInputPorts().getPortByIndex(0).getRawMetaData()); // no attributes will
						// be removed, just deliver old MD
						return;
					}
					...
					if (getParameterAsString(PARAMETER_ORDER_MODE).equals(REFERENCE_DATA)) {
						ExampleSetMetaData refMD = getInputPorts().getPortByName(REFERENCE_DATA_PORT_NAME).getMetaDataAsOrNull(ExampleSetMetaData.class);
						if (refMD == null) {
							outputPort.deliverMD(new ExampleSetMetaData());
							return;
						}
						List<AttributeMetaData> allReferenceAttributes = new LinkedList<>(refMD.getAllAttributes());
					...
					if (getParameterAsString(PARAMETER_ORDER_MODE).equals(USER_SPECIFIED_RULES_MODE)) {
						...
						for (int i = 0; i < splittedRules.length; i++) {
							rules.add(StringTools.unmask('|', splittedRules[i]));
						}
					...
					//need to use deprecated method because applyRulesOnMetaData does not expect TableMetaData
		...
		if (getParameterAsString(PARAMETER_ORDER_MODE).equals(ALPHABETICALLY_MODE)) {
			...
			if (getParameterAsString(PARAMETER_ORDER_MODE).equals(REFERENCE_DATA)) {
				...
				for (String maskedRule : combinedMaskedRules.split("\\|")) {
					String rule = StringTools.unmask('|', maskedRule); // unmask them to allow regexp
