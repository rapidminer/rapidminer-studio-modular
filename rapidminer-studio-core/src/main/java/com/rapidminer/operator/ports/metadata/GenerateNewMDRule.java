...
public class GenerateNewMDRule implements MDTransformationRule {
	...
	public GenerateNewMDRule(OutputPort outputPort, Class<? extends IOObject> clazz) {
		this(outputPort, forClass(clazz));
	}
	...
	/**
	 * Modifies the standard meta data before it is passed to the output. Can be used if the transformation depends on
	 * parameters etc. The default implementation just returns the original. Subclasses may safely modify the meta
	 * data,
	 * since a copy is used for this method.
	 */
	...
	/**
	 * Returns {@link ExampleSetMetaData} for {@link ExampleSet}, {@link TableMetaData} for {@link IOTable} and just
	 * {@link MetaData} with the clazz otherwise.
	 */
	private static MetaData forClass(Class<? extends IOObject> clazz) {
		if (ExampleSet.class.equals(clazz)) {
			return new ExampleSetMetaData();
		} else {
			return IOTable.class.equals(clazz) ? new TableMetaData() : new MetaData(clazz);
		}
	}
