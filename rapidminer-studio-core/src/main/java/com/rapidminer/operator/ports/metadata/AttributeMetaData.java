...
public class AttributeMetaData implements Serializable {
		...
		if (valueRange != null && Ontology.ATTRIBUTE_VALUE_TYPE.isA(getValueType(), Ontology.DATE_TIME)
			...
			switch (getValueType()) {
				case Ontology.DATE:
					...
					break;
				case Ontology.TIME:
					buf.append(Tools.formatTimeWithMillis(new Date((long) getValueRange().getLower())));
					buf.append("...");
					buf.append(Tools.formatTimeWithMillis(new Date((long) getValueRange().getUpper())));
					buf.append("]");
					break;
	...
	/**
	 * @deprecated since 9.8; attribute annotations might not be available in the future
	 */
	@Deprecated
