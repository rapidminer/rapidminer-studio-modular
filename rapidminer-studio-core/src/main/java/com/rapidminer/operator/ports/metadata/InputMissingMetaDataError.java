	...
	public List<QuickFix> getQuickFixes() {
		...
		if (desiredClass != null) {
			for (OutputPort outputPort : inputPort.getPorts().getOwner().getConnectionContext().getAllOutputPorts()) {
				if (!outputPort.isConnected() && outputPort.getRawMetaData() != null
						&& (desiredClass.isAssignableFrom(outputPort.getRawMetaData().getObjectClass())
						|| AtPortConverter.isConvertible(outputPort.getRawMetaData().getObjectClass(), desiredClass))) {
