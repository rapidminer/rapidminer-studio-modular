...
/**
 * Wraps a {@link Model} into an {@link IOTableModel}.
 *
 * @author Gisa Meier
 * @since 9.10
 */
public class WrappedModel extends AbstractIOTableModel implements  WrappedGeneralModel {

	private final Model model;

	public WrappedModel(Model model) {
		this.model = ValidationUtilV2.requireNonNull(model, "model");
	}

	/**
	 * Returns the wrapped {@link Model}.
	 *
	 * @return the wrapped model
	 */
	@Override
	public Model getDefiningModel() {
		return model;
	}
	...
	public IOTable getTrainingHeader() {
		return getHeaderTable(model.getTrainingHeader());
	}
	...
	static IOTable getHeaderTable(HeaderExampleSet header) {
		if (header == null) {
			return null;
		}
		return new IOTable(BeltConverter.convert(header, new SequentialConcurrencyContext())
				.getTable().stripData());
	}
}
