		...
		do {
			...
			for (InputPort in : inputPorts.getAllPorts()) {
				...
				if (!in.isConnected() && !complete.contains(in)
					...
					Iterator<OutputPort> outIterator = readyOutputs.descendingIterator();
					while (outIterator.hasNext()) {
						...
						if (owner.shouldAutoConnect(outCandidate) && outCandidate.getRawMetaData() != null &&
								in.isInputCompatible(outCandidate.getRawMetaData(), level)) {
