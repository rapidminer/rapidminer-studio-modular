...
public class LeastSquareDistributionColumnCriterion extends LeastSquareColumnCriterion {
	...
	public double getNominalBenefit(ColumnExampleTable columnTable, int[] selection, int attributeNumber) {
		...
		return makeFinalBenefit(difference, totalResidual, selection.length);
	}
	...
	protected double calculateNumericalBenefit(int[] selection, double splitValue, double[] numericalAttributeColumn, double[] label, double[] sums, double[] averages) {
		...
		return makeFinalBenefit(residualDifference, totalResidual, selection.length);
	}

	/**
	 * Hook for subclass {@link LeastSquareDistributionColumnCriterion} to change this calculation.
	 */
	protected double makeFinalBenefit(double residualDifference, double totalResidual, double selectionLength){
		return (residualDifference) / (totalResidual * selectionLength);
	}
	...
	public double getIncrementalBenefit(WeightDistribution distribution) {
		...
		return makeFinalBenefit(difference, totalResidual, lsDistribution.getLength());
	}
