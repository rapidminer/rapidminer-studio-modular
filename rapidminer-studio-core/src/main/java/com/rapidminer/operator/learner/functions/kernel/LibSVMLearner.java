...
public class LibSVMLearner extends AbstractKernelBasedLearner {
		...
		try {
			...
			switch (lc) {
				...
				case POLYNOMINAL_LABEL:
					return type == SVM_TYPE_NU_SVC || type == SVM_TYPE_C_SVC || type == SVM_TYPE_ONE_CLASS;
				case ONE_CLASS_LABEL:
					return type == SVM_TYPE_ONE_CLASS;
				case NUMERICAL_LABEL:
					return type == SVM_TYPE_NU_SVR || type == SVM_TYPE_EPS_SVR;
				default:
			}
		} catch (UndefinedParameterError e) {
		}
	...
	/**
	 * Checks if the label can be used and creates a support vector problem for the LibSVM either using or ignoring the
	 * label.
	 */
	private svm_problem getProblem(ExampleSet exampleSet, svm_parameter params) throws UserError {
		log("Creating LibSVM problem.");
		FastExample2SparseTransform ripper = new FastExample2SparseTransform(exampleSet);
		Attribute label = exampleSet.getAttributes().getLabel();
		boolean oneClassSVM = params.svm_type == SVM_TYPE_ONE_CLASS;
		boolean useLabel = !oneClassSVM || label.getMapping().size() == 1;
		...
		if (useLabel) {
			...
			// using the first label value as dummy for one class svm
			double dummyLabel = exampleSet.getExample(0).getValue(label);
			for (Example e : exampleSet) {
				problem.x[j] = makeNodes(e, ripper);
				problem.y[j] = dummyLabel;
				nodeCount += problem.x[j].length;
				j++;
			}
		}
	...
	public Model learn(ExampleSet exampleSet) throws OperatorException {
		...
		svm_problem problem = getProblem(exampleSet, params);
		this.checkForStop();
		String errorMsg = Svm.svm_check_parameter(problem, params);
		if (errorMsg != null) {
			throw new UserError(this, 905, "libsvm", errorMsg);
		}
		log("Training LibSVM.");
